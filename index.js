	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				error: {
					stick: 'DEU NAO AMG TENTA DNV',
					Iv: 'link invalido, tente novamente'
				},
				only: {
					group: 'USE ESSE COMANDO NO GRUPO!',
					ownerG: 'SOMENTE OS ADMINISTRADORES DO GRUPO PODEM USAR ESTE COMANDO!',
					ownerB: 'SOMENTE O DONO DO BOT PODE USAR ESSE COMANDO',
					admin: 'SOMENTE O DONO DO BOT PODE USAR ESTE COMANDO!',
					Badmin: 'SEJA ADMINISTRADOR DESTE GRUPO PARA USAR ESTE COMANDO! '
				}
			}

			const botNumber = client.user.jid
			const ownerNumber = ["5512991065344@s.whatsapp.net"] // COLOQUE SEU NÚMERO AQUI SEM ESPAÇOS
...
//AQUI NAO PRECISA MEXER
	...
	//AQUI NAO PRECISA MUDAR
				...
				case 'ytsearch':
					if (args.length < 1) return reply('NOS EXPLIQUE O QUE VOCE QUER PESQUISAR')
				...
				case 'stalk':
					try {
						...
						console.log(`Error :`, color(e,'red'))
						 reply ('NOME OU "@" INVALIDO!')
					}
					break
			
				case 'nulis':
				case 'tulis':
					if (args.length < 1) return reply('OQ VOCE QUER ESCRITO?')
					teks = body.slice(7)
					reply(mess.wait)
	...
	//AQUI NAO PRECISA MUDAR
			...
			case 'tagall':
				...
				case 'clearall':
					...
					reply('TODOS OS CHATS FORAM LIMPOS')
					break
				
				case 'bc':
					if (!isOwner) return reply('QUEM É?') 
					...
					if (mentioned.length > 1) {
						teks = 'RESPEITEM ESSE GAY AQUI QUE ELE ACABA DE GANHAR ADM🤡🙌\n'
				...
				case 'demote':
					...
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'NAO ACEITOU O GF E PERDEU ADM \n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						...
						mentions(`REBAIXOU @${mentioned[0].split('@')[0]} COM EXITO!`, mentioned, true)
				...
				case 'add':
					...
					if (args[0].startsWith('08')) return reply('ESCREVA O NUMERO CORRETAMENTE EX: 5521997341559')
					try {
						num = `${args[0].replace(/ /g, '')}@s.whatsapp.net`
						client.groupAdd(from, [num])
					} catch (e) {
						console.log('Error :', e)
						reply('NUMERO PRIVADO, NAO FOI POSSIVEL ADICIONÁ-LO' )
					}
					break
				...
				case 'kick':
					...
					if (!isGroupAdmins) return reply(mess.only.admin)
					if (!isBotGroupAdmins) return reply(mess.only.Badmin)
					if (mek.message.extendedTextMessage === undefined || mek.message.extendedTextMessage === null) return reply('MARCA ELE LIXO!')
					mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid
					if (mentioned.length > 1) {
						teks = 'NAO MAMOU O ADM E FOI EXPULSO😼:\n'
						for (let _ of mentioned) {
							teks += `@${_.split('@')[0]}\n`
						}
						mentions(teks, mentioned, true)
						...
						mentions(`NAO MAMOU O ADM E FOI EXPULSO😼: @${mentioned[0].split('@')[0]}`, mentioned, true)
						client.groupRemove(from, mentioned)
					}
					break
//AQUI NAO PRECISA MUDAR		
...
//AQUI NAO PRECISA MUDAR                   
				...
				case 'toimg':
					if (!isQuotedSticker) return reply('MARCA A FIGURINHA AI IDIOTA!')
					reply(mess.wait)
					...
					exec(`ffmpeg -i ${media} ${ran}`, (err) => {
						fs.unlinkSync(media)
						if (err) return reply('HOUVE UMA FALHA, TENTE NOVAMENTE MAIS TARDE!')
...
//NAO FUNCIONA MAIS	
				...
				case 'welcome':
					...
					if (Number(args[0]) === 1) {
						...
						fs.writeFileSync('./src/welkom.json', JSON.stringify(welkom))
						reply('MENSAGEM DE BOAS VINDAS ATIVADA')
					} else {
						reply('1 untuk mengaktifkan, 0 untuk menonaktifkan')
					}
                                      break
